...
def ConfToMol(mol, conf):
    ...
    mol_.AddConformer(Chem.Conformer(conf), assignId=False)
    return mol_
...
def DBSTEP_Properties(struc, probe, id, prox_radius=3.5):
    ...
    for conf in probe.GetConformers():
        ...
        if not prbs:
            prbs = Chem.CombineMols(struc, mol_)
        else:
...
def RDKit_Properties(struc, probe):
    ...
    for conf in probe.GetConformers():
        ...
        #print(mol_.GetConformer())
        if not cmplx:
            cmplx = Chem.CombineMols(struc, mol_)
        else:
            cmplx = Chem.CombineMols(cmplx, mol_)
    ...
    properties['RDKit_Asphericity_cavity'] = Descriptors3D.Asphericity(prbs)
    properties['RDKit_Eccentricity_cavity'] = Descriptors3D.Eccentricity(prbs)
    properties['RDKit_InertialShapeFactor_cavity'] = Descriptors3D.InertialShapeFactor(prbs)
    properties['RDKit_SpherocityIndex_cavity'] = Descriptors3D.SpherocityIndex(prbs)
    properties['RDKit_NormalizedInertiaRatio1/3_cavity'] = Descriptors3D.NPR1(prbs)
