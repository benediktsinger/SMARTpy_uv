...
def rotation(probe, conf, tip, rotvec, theta):
    for i in range(probe.GetNumAtoms()):
        a = np.array(conf.GetAtomPosition(i))
        dotproduct = rotvec[0]*(float(a[0]) - float(tip[0])) + rotvec[1]*(float(a[1]) - float(tip[1])) + rotvec[2]*(float(a[2]) - float(tip[2]))
        centre = [float(tip[0]) + dotproduct*rotvec[0], float(tip[1]) + dotproduct*rotvec[1], float(tip[2]) + dotproduct*rotvec[2]]
        v = [float(a[0]) - centre[0], float(a[1]) - centre[1], float(a[2]) - centre[2]]
        d = math.sqrt(v[0]*v[0]+v[1]*v[1]+v[2]*v[2])
...
def custom_template(step=1):
    ...
    if PARAMS.BINDING_POS is not None and PARAMS.PROBE_MOL is not None:
        probe = Chem.RWMol(PARAMS.PROBE_MOL)
        probe.RemoveAtom(probe.GetNumAtoms()-1)
    else:
        frags = Chem.GetMolFrags(CONFS.CPLX, asMols=True)
        probe = frags[0]
...
def template_search(step=1):
    ...
    if PARAMS.BINDING_POS is not None and PARAMS.PROBE_MOL is not None:
        ...
        #probe = PARAMS.PROBE_MOL
        structure = MOL_INIT.MOL
    else:
        frags = Chem.GetMolFrags(CONFS.CPLX, asMols=True)
        probe = frags[0]
        structure = Chem.DeleteSubstructs(CONFS.CPLX, probe)
    ...
    for p in TEMPLATE.MOLS.GetConformers():
        probe_upd = ConfToMol(TEMPLATE.MOLS, p)
        if PARAMS.BINDING_POS is not None:
            translation_v =  PARAMS.BINDING_POS - p.GetAtomPosition(0)
        else:
            translation_v =  probe.GetConformer().GetAtomPosition(0) - p.GetAtomPosition(0)
        for i in probe_upd.GetAtoms():
            pt = np.array(p.GetAtomPosition(i.GetIdx()))
            #print(pt)
            translate = pt+translation_v
            p.SetAtomPosition(i.GetIdx(), translate)
        ...
        rot = structure.GetConformer().GetAtomPosition(0) - p.GetAtomPosition(0)
        rotn = np.linalg.norm(rot)
        rotvec = ( rot / rotn )
        rotation(probe, p, probe.GetConformer().GetAtomPosition(0), rotvec, RANDOM_ROT)
        #print('rotation')
