...
def get_chimera_descriptors(mol_file, cavity_file, path=os.getcwd(), radius=3.5, lvl_mol=0.07, lvl_cav=4):
    cavity_path = os.path.join(path, cavity_file)
    mol_path = os.path.join(path, mol_file)

    try:
        rc("open "+cavity_path) #0
        rc("shape sphere center @S1 radius "+str(radius)) #1
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', default=3.5, required=False) #proximal radius
